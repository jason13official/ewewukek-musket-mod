...
//    }
    ...
    public static boolean poseArm(LivingEntity entity, HumanoidModel<? extends HumanoidRenderState> model, ModelPart arm) {
    ...
    public static void renderGunInHand(ItemInHandRenderer renderer, AbstractClientPlayer player, InteractionHand hand, float dt, float pitch, float swingProgress, float equipProgress, ItemStack stack, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        ...
        if (!gun.canUseFrom(player, hand)) {
            ...
            renderer.renderItem(player, stack, isRightHand ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, poseStack, bufferSource, light);
            poseStack.popPose();
            return;
        }
